...
public class Coord {
	...
	*/
	...
	public boolean lineIntersectSegment(Coord cOth, Coord seg1, Coord seg2)
	{
	// returns true if infinite line from current Coord to cOth intersects segment from seg1 to seg2 in point OTHER than one of the vertices
	...
	return (cI.contained(lowX2,lowY2,highX2-lowX2,highY2-lowY2) );
	}
	...
	public boolean isVertical(Coord cOth)
	{
	Vector2Do coordsVector = new Vector2Do(this,cOth);
	return coordsVector.isVertical();
	}
	...
	Vector2Do coordsVector = new Vector2Do(cIn,cOut);
	double cSlope = coordsVector.getSlope();
	Coord result;
	...
	{
		Vector2Do cornerVector = new Vector2Do(0,0,cOut.getX(),cOut.getY());
	...
	{
		Vector2Do cornerVector = new Vector2Do(boardWidth,0,cOut.getX(),cOut.getY());
		double cornerSlope = Math.abs(cornerVector.getSlope());
	...
	{
		Vector2Do cornerVector = new Vector2Do(boardWidth,boardHeight,cOut.getX(),cOut.getY());
		double cornerSlope = Math.abs(cornerVector.getSlope());
	...
	{
		Vector2Do cornerVector = new Vector2Do(0,boardHeight,cOut.getX(),cOut.getY());
		double cornerSlope = Math.abs(cornerVector.getSlope());
	...
	Vector2Do corners[] = new Vector2Do[4];
	...
	for (int i=0; i<4; i++)
	{
	corners[i] = new Vector2Do(coordCorners[i],cOut); 
	...
	*/
	...
	{
	Coord A = new Coord(182.0, 175.0);
	Coord B = new Coord(376.0, 192.0);
	Coord C = new Coord(314.0, 278.0);
	Coord cent = new Coord(278.67702108467705, 187.1857593866271);
	Coord D = new Coord(362.0, 124.0);
	System.out.println(A.lineIntersectSegment(cent,B,D));
	System.out.println(A.intersection(cent,B,D));
	}
}